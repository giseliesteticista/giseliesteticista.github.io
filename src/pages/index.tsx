      ...
      if(service == 'retoque' || service == 'micropigmentacao') {
        ...
        if(nome && service && telefone && hour && moment(data).day() + 1 != 1 && moment(data).day() + 1 != 2) {
          ...
          if(response.status == 201) {
            ...
            window.localStorage.setItem('hasAppointment', JSON.stringify(response.data.data));
      ...
      <main className={styles.main}>
        ...
        <div className={styles.appointmentForm}>
          ...
          <input 
            ...
            value={data}
            onChange={(txt) => {
              if(moment(txt.target.value).day() + 1 == 1 || moment(txt.target.value).day() + 1 == 2) {
                alert('Não atendemos aos domingos e segundas. Escolha uma data diferente!')
              } else {
                setData(txt.target.value);
                getFreeAppointments(txt.target.value);
              }
            }}
          />
